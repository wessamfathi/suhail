...
6. Write `.northstar/state.json`. Regen STATUS.md. Narrate: "Initialized with N Parts across L levels — scouting level 0 (M Parts) in parallel." Re-tick into `batch_scouting`.
...
**Discard rule:** rename `brief.md` and `brief-*.md` under the speculative Part to `*.speculative.md` via `Rename-Item` / `mv`. Clear `state.speculative = null`. Do NOT touch other artifacts. Narrate: "Discarded speculative artifacts for Part M."

**Adopt rule:** when ticking into Part M's `scouting` handler — if `state.speculative.part_id == "part-<M>"` AND `brief.md` exists: skip dispatch, go directly to output verification, clear `state.speculative`. Narrate: "Adopted speculative brief for Part M — skipping re-scout."
...
Derive the current level integer from the `level` field of any Part in `current_batch`. Append that integer to `batch_scouted_levels`, write `state.json`. Emit all scout `Agent(...)` calls for `current_batch` (integer-sorted) in one assistant turn. Narrate: "Scouting level L — dispatching M scouts in parallel: Part a, Part b, …"
...
```
Narrate: "Part N is trivial — skipping scout." Go directly to external-deps checkpoint (step 5 below).

1. Adopt rule check (see Speculative dispatch).
2. Slice plan file to extract Part body.
3. Dispatch scout `Agent(...)` (same shape as `start_batch_scouting`).
4. Output verification. On failure → `needs_user`, blocker.md, end turn.
...
- `Approve and run to end` → compute `last_part_id` (max integer in `parts`). Set `mode = "run-to"`, `run_to = <last_part_id>`, `auto_approve_planner = true`. Narrate: "Run-to-end activated." Set status `executing`, re-tick.
...
3. Dispatch:
   ...
   ```
4. Output verification for `review.md` and `audit.md`. On failure → blocker.md, `needs_user`, end turn.
...
3. If no pending Part with all deps terminal: run is finished. Regen STATUS.md. AskUserQuestion: "All Parts completed. Options: Show summary / Abort (clears state) / Done."
...
6. Branch on mode:
   - `autorun`: increment session-local counter `autorun_parts_completed` (starts at 0, not persisted to `state.json`). If counter >= 10: narrate "Autorun safety cap reached — use `/ns continue` to proceed." End turn. Otherwise: suppress AskUserQuestion, re-tick. (After a blocker is resolved via `/ns continue`, `mode` remains `"autorun"` in `state.json` so the next tick re-enters this branch.)
   ...
   - `interactive`: B5 speculative scout for next Part in `current_batch`. AskUserQuestion: "Part N complete. Continue to Part M?" (options: `Continue / Pause / Commit first / Show diff / Show review / Show audit / Run to end`). On `Abort`: Discard rule then abort normally. On `Run to end`: keep speculative, set `mode = "run-to"` to `last_part_id`, narrate, re-tick.

### `needs_user`
Read latest `blocker.md` in `current_part_id`'s directory. AskUserQuestion with `options` plus "Other (free text)". On next invocation: append `resolution: <answer>` to blocker.md (Edit). Record in `global_decisions`. Set status back to the phase that raised the blocker (`from:` field: `scout` → `scouting`, `executer` → `executing`, `verifier` → `verifying`, `orchestrator` → retry current phase). Re-tick.

### `aborted`
Emit one-sentence confirmation. End turn.

### `noop`
Narrate `directive.reason`. AskUserQuestion: "Unexpected state — continue or abort?" (options: `Continue / Abort`).
...
One short sentence before each dispatch and one after pointing at the artifact. Never verbose, never silent. Do not echo subagent prompt content or artifact bodies.
