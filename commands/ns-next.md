...
5. Locate `northstar.md` using the two-location lookup described below, read it, and follow its instructions for **exactly one tick** as if the orchestrator had been invoked with empty arguments. The orchestrator's `awaiting_plan_approval` branch checks `auto_approve_planner` via the planning-phase short-circuit at `northstar.md` "If `auto_approve_planner == true` (run-to mode): advance to `executing`. Re-tick." — but since we are already at `awaiting_plan_approval` (past the planning phase), the equivalent transition here is the "Approve" branch: set `current_step = "executing"`, then re-tick.
...
1. Locate `northstar.md` using the two-location lookup below.
...
2. If neither path contains the file, end with: "Cannot locate `northstar.md` — reinstall Northstar."

Do not duplicate or summarize the orchestrator logic here. The canonical state machine lives in `northstar.md`.
