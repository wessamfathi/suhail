...
AskUserQuestion with `options` plus "Other (free text)". On next invocation: append `resolution: <answer>` to blocker.md (Edit). Record in `global_decisions`. Set status back to the phase that raised the blocker (`from:` field: `su-scout` → `scouting`, `su-executer` → `executing`, `su-verifier` → `executed`, `orchestrator` → retry the phase that raised it — and for ANY blocker raised during verification, whether from `su-verifier` or the orchestrator's own verdict check, a Retry resolution sets status `executed` so verification re-dispatches via `start_batch_verifying`; its resume shortcut ignores artifacts with null verdicts, so the malformed files are overwritten rather than re-adopted). Re-tick.
