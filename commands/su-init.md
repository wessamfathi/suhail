...
`/su` and `/su-discover` refuse to run until this intel exists. Run `/su-init` once per project. Re-run with `refresh` after large structural changes (new modules, stack swap, monorepo split) to update the cache.
...
The `su-indexer` subagent writes four files under `.suhail/intel/`:
...
3. **Existence check.** Look for `.suhail/intel/stack.md`, `.suhail/intel/layout.md`, `.suhail/intel/conventions.md`, `.suhail/intel/modules.md`.
   ...
   - If all four exist AND `$ARGUMENTS != "refresh"`: end with AskUserQuestion: "Project intel already exists at `.suhail/intel/`. Options: Refresh (rescan and overwrite) / Skip (keep current) / Show summary."
...
4. **Prepare directories.** Ensure `.suhail/`, `.suhail/intel/`, and `.suhail/plans/` exist. Use Bash:
   - POSIX: `mkdir -p .suhail/intel .suhail/plans`
   - Windows: `New-Item -ItemType Directory -Path .suhail/intel,.suhail/plans -Force | Out-Null`
...
6. **Dispatch the indexer.** Resolve the repo root via Bash (`pwd` on POSIX, `$PWD` on PowerShell) before dispatch so you can pass an absolute path.
   ```
   Agent(
     subagent_type="su-indexer",
     description="Scan project intel",
     prompt="""
   Output directory: .suhail/intel/
...
7. **Output verification.** After return, FIRST check for an indexer blocker — before any sentinel check, because the indexer's contract writes blocked STUB intel files that would otherwise satisfy the checklist: if `.suhail/intel/blocker.md` exists without a `resolution:` line, OR any intel file's body is the blocked marker `Blocked — see blocker.md`, surface the blocker via AskUserQuestion using the options in its frontmatter plus "Other (free text)" and end the turn — do NOT declare success. Otherwise run the following verification checklist. For each file:
   ...
   |---|---|
   | `.suhail/intel/stack.md` | `## Languages and frameworks` AND `## Commands` |
   | `.suhail/intel/layout.md` | `## Top-level layout` |
   | `.suhail/intel/conventions.md` | `## House conventions` |
...
8. **Success.** Emit one short sentence per written file pointing at its path:
   - "📦 Indexer — done — see `.suhail/intel/stack.md`."
   - "📦 Indexer — done — see `.suhail/intel/layout.md`."
   - "📦 Indexer — done — see `.suhail/intel/conventions.md`."
   - "📦 Indexer — done — see `.suhail/intel/modules.md`."

## Re-run on resolution

If the user picked "Retry the indexer" on a blocker, append `resolution: Retry` to `.suhail/intel/blocker.md` via Edit, then re-dispatch (step 6) once. Cap retries at 2 — on third failure, escalate with a different option set: `["Show what the indexer wrote", "Abort", "Open the blocker"]`.
...
- Don't run when `.suhail/state.json` shows an in-flight run (not aborted and `run_phase` not terminal) — you would race with the orchestrator. A `finished`/`completed`/`aborted` run is terminal and safe to run over.
