...
2. Pick the target Part: the Part whose `id == current_part_id` — unless that Part is already terminal (`completed`/`skipped`), in which case target `next_eligible_part(for_batch_only=false)` as defined in `su.md`'s Speculative-dispatch section (the lowest-integer pending Part whose dependencies are all completed or skipped; this is the Part a "skip" at a level checkpoint means). Set the target's `status: "skipped"` and point `current_part_id` at it. A skipped Part is terminal, so on the next tick the batch routing (`batch_directive`) simply proceeds past it to the next dispatchable Part (or to `complete` when the whole batch is terminal); you do not compute anything beyond the target yourself.
