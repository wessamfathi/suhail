...
2. Set the Part whose `id == current_part_id` to `status: "skipped"`. Leave `current_part_id` pointing at it — a skipped Part is terminal, so on the next tick the batch routing (`batch_directive`) simply proceeds past it to the next dispatchable Part (or to `complete` when the whole batch is terminal); you do not compute the next Part yourself.
